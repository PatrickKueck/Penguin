...
# v1.34 bug in generating quartets from clade-definition taxa infile order (line 1628,1629)
# v1.35 bug in -p definition. If -p undefined, wrong parameter value adapted from next line in %default_parameter_setting (line 202), singleton sort command in phyquart analysis
# v1.36 -> defined as first full version v1.0
...
my %info_of_author			=(
								'name'			=>	'Patrick Kueck'			,
								'institute'		=>	'NHM London, UK'		,
								'email'			=>	'pkuck[at]nhm.ac.uk'	,
								'homepage'		=>	'www.nhm.ac.uk'			,
...
my %grafik_colours			= (
								'0'							=>	'blue'		,	# split and brachart color of     best supported quartet topology -> blue
								#'0'							=>	'#99CCFF'	,	# split and brachart color of     best supported quartet topology -> light blue
								'1'							=>	'red'		,	# split and brachart color of 2nd best supported quartet topology -> red
								#'1'							=>	'#FF9999'	,	# split and brachart color of 2nd best supported quartet topology -> light red
								'2'							=>	'green'		,	# split and brachart color of 3rd best supported quartet topology -> green
								#'2'							=>	'#009900'	,	# split and brachart color of 3rd best supported quartet topology -> light green
) ;
...
my %graphic_title			=(
								...
								'barN'						=>	"Penguin N Quartet Topologies"				,	# Title for SVG Barchart outfile name for N distribution best, 2nd, and 3rd quartet signal
								'spliN'						=>	"Penguin N Quartet Topologies"				,	# Title for SVG Split outfile name for N distribution best, 2nd, and 3rd quartet signal
) ;
#######################
...
my %text_title				=(
								...
								'triall'					=>	$scriptname.'_triangle_qsingle_spl_sup.svg'	,	# Triangle output for splitsupport of single quartet analyses
								'tritax'					=>	$scriptname.'_triangle_tsingle_mea_sup.svg'	,	# Triangle output for splitsupport of single taxa analyses
								'trimed'					=>	$scriptname.'_triangle_tsingle_med_sup.svg'	,	# Triangle output for splitsupport of single taxa analyses
								...
								'barN'						=>	$scriptname.'_bar_qnumber.svg'				,	# Barchart outfile name for N distribution best, 2nd, and 3rd quartet signal. Name sufficx will be extended for print out
								'barN'						=>	'none'										,	# If none, no print of barchart files. For print comment out this line
								'spliN'						=>	$scriptname.'_split_qnumber.svg'			,	# Split outfile name for N distribution best, 2nd, and 3rd quartet signal
								'txtbsupport'				=>	$scriptname.'_qnumber.txt'					,	# Text print of quartets, sorted by best, second best and worst support
								'txtbsupport'				=>	'none'										,	# If none, no print of taxa combinations found for best and second best split support. For print comment out this line
...
print	"\n\t___________________________________________________",
		"\n\t---------------------------------------------------",
		"\n\t", $scriptname ,
		"\n\tIdentification Of Synapomorph Split Signal In Quartets",
		"\n\t",
		"\n\tWritten by Patrick Kueck (NHM London, 2015/16)" ,
		"\n\tE-mail: pkuck\@nhm.ac.uk" ,
		"\n\t---------------------------------------------------\n"
;
#######################
...
my %default_parameter_setting = (
						...
						'l' => 1000		,	# def: 10000	<integer> (Max Quartet limit)	skip from 10000 (default) to defined value
						...
						'v' => 1		,	# def: 0		<integer> (AA handling)			skip from 0 -> aa handling disabled to 1 -> aa handling enabled (aa handling currently not fully implemented)
...
rename ( $text_title{tmpinfo}, $name_of_subfolder{txt}."/".$text_title{txtinfo} ) ;
...
sub help{
	...
	if		( $$sref_command eq 'h' ){
		...
		print	"\n\tML Substitution Model Parameter:" ,
				"\n\t-----------------------------------",
				"\n\tAllowed Format:",
				"\n\t- String (Default: ", $href_default_set->{m} ," (NUC)",
				"\n\t",
...
sub argv_handling{
	...
	if ( $commandline ){ 
		...
		for my $command ( @commands ){
			...
			elsif 	( 	$command =~ /^N$|^c$|^u$/		)																							{ $href_par_set_of_opt->{$command}	= 1					}
			...
			elsif 	( (	$command =~ /^I$/				) && ( $parameter =~	/^\d+.\d+$/														) )	{ $temp_value_of_opt{$command}		= $parameter		}
			elsif 	( (	$command =~ /^m$/				) && ( $parameter =~	/^JC$|^GTR$|^F81$|^K2P$|^HKY$|^d78$|^jtt$|^wag$|^mtrev24$/i		) )	{ $href_par_set_of_opt->{$command}	= $parameter		}
			elsif 	( (	$command =~ /^M$|^l$/			) && ( $parameter =~	/^\d+$/															) )	{ $href_par_set_of_opt->{$command}	= $parameter		}
			
			elsif 	( 	$command =~ /^h$/i				){
				
				if ( $parameter =~ /^p$|^N$|^c$|^i$|^a$|^I$|^m$|^M$|^l$|^P$|^u$/ )	{ &help( \$parameter, \$$sref_script, \%$href_par_set_of_opt, \%$href_info_author, \%$href_def_par_setting ) }
		...
		if		(	(	$temp_value_of_opt{I} < 0 )	||
					(	$temp_value_of_opt{I} > 1 )	){
			
			print "\n\tPARAMETER-ERROR: Invariabel site proportion must be between 0 and 1!\n" ; &help( \'I', \$$sref_script, \%$href_par_set_of_opt, \%$href_info_author, \%$href_def_par_setting )
		}
		elsif	(	$temp_value_of_opt{I} ){ $href_par_set_of_opt->{I}	=	$temp_value_of_opt{I} }
		#######################
	...
	unless (	($href_par_set_of_opt->{N} == 0 ) && ( $href_par_set_of_opt->{n} == 1 ) ){
		
		print OUTinfo	$hoh_print_of_parameter{N}{$href_par_set_of_opt->{N}} ;
		print 			$hoh_print_of_parameter{N}{$href_par_set_of_opt->{N}}
	}
	else {
	...
	for my $taxon ( sort keys %$href_unred_seq_of_taxon ){
		...
		for my $state_pos ( 0 .. @seq_states-1 ){ 
			...
			unless ( $href_info_of_property->{type} eq 'aa' ){
				...
				if ( $seq_states[$state_pos] =~ /E|L|Q|F|P|X/ ){ 
					
					$href_info_of_property->{type} = 'aa' ;
					
					################################################################################################################### AA handling check !!
					if ( $href_setting_of_option->{v}	== 0 ){ print "\n\tINFILE-ERROR: Forbidden sign (", $seq_states[$state_pos], ") in sequence of ", $taxon, "!\n"; &help( \'i', \$$sref_scriptname, \%$href_setting_of_option, \%$href_info_author, \%$href_def_par_setting ) }
					###################################################################################################################
				}
			}
			#######################
...
sub readin_fasta{
	...
	while ( my $line = <INfas> ){
		...
		else { $seq_of_tax{$taxon} = $seq_of_tax{$taxon}.$line }
		#######################
	}
...
sub readin_phylip{
	...
	for my $line (@all_lines_phy){ if ( $line =~ /\w+/ ){ push @cleaned_lines_phy, $line } }
...
sub perform_quartet_analyses{
	...
	my (
			...
			%topology_of_quartet_number ,	# key: quartet number; value: best topology between subclades
	...
	for my $quartet_number ( 1 .. @$aref_quartets ){
		...
		# terminal print
		my @q_taxa = split ":", @$aref_quartets[$quartet_number-1] ;
		my $quartet_taxa	= join ":", @q_taxa ;
		
		if ( $href_parameter_setting_of_option->{N} == 0 ){
			
			print	OUTrej		"Split Analysis q", $quartet_number, ":\t", $quartet_taxa ;
		}
		print				"\n\tStart Split Analysis q", $quartet_number, ": ", $quartet_taxa ;
		#######################
		...
		if ( $href_parameter_setting_of_option->{N} == 0 ){
					...
					print OUTrej 	"\tRejected Quartet Positions:\t",		$N_rejected_pos_quartet , "\tRemaining Quartet Positions:\t",	$remaining_pos_total ;
		...
		else { $remaining_pos_all	+= $remaining_pos }
		############################################################################################ END Overall Alignment Exclusion
		...
		my (
				%reduced_seq_of_tax ,				# key: quartet taxon; value: reduced sequence
				%spin_value_of_quartet_topology ,	# key: quartet_topology; value: spin_value
		);
		...
		&start_phyquart(
									...
									\$quartet_number						,	# quartet name																			In -> Defined, OUT -> Unchanged
									...
									\%spin_value_of_quartet_topology		,	# key: quartet_topology; value: spin_value												In -> Undefined, OUT -> defined
		...
		#################################################################################################################### START MAIN QUARTET RESULT ASSIGNMENT
		
		
		
		################################
		# Sort quartet topologies by values and store them as list variabel
		my @sorted_scored_top	=	sort {$spin_value_of_quartet_topology{$b} <=> $spin_value_of_quartet_topology{$a}} keys %spin_value_of_quartet_topology ;
		################################
		
		
		
		################################
		# Extract taxa of each quartet topology
		# Sort sister group relationships of each topology by sorting taxon names of each sistergroup
		# Count number of best, second, and third best quartet topology
		# store distance values between sistergroups by dividing 1/split score. So that the highest supported sister group relationship has the lowest distance value
		# ...only best split quartet in %hoh_dist_1st_cla1_cla2 (both other split values are set to 1)
		# ...best and second best quartet in %hoh_dist_1st_2nd_cla1_cla2 (last split value set to 1)
		# store single split scores of each quartet as list in %hol_values_of_quartet
		print "\n\tAssign Split Results...\n\t---------------------------------------------------\n" ;
		
		for my $topo_quartet (0 .. 2){
			
			( my $top_q	= $sorted_scored_top[$topo_quartet] )				=~	s/(\()+|(\))+//g ;
			my	@taxa														=	split ",", $top_q ;
			my	@sister_le													=	sort ( $subclade_of_taxon{$taxa[0]}, $subclade_of_taxon{$taxa[1]} ) ;
			my	@sister_ri													=	sort ( $subclade_of_taxon{$taxa[2]}, $subclade_of_taxon{$taxa[3]} ) ;
			my	$sister_le													=	join ",", @sister_le ;													#print "\n", $sister_le, "\n"; 
			my	$sister_ri													=	join ",", @sister_ri ;													#print "\n", $sister_ri, "\n"; exit;
			my	@sort_sister												=	sort ( $sister_le, $sister_ri ) ;
			my	$topology													=	"(".$sort_sister[0]."),(".$sort_sister[1].")" ;
			$seen_quartets_all			{$topology}++ ;
			$hoh_seen_quartet			{$topo_quartet}{$topology}++ ;
			$topology_of_quartet_number	{$quartet_number}					= $topology ;
			...
			if ($topo_quartet == 1){
				...
				$hoh_dist_1st_cla1_cla2		{$sister_le[0]}{$sister_le[1]}		= $spin_value_of_quartet_topology{$sorted_scored_top[2]} ;
				$hoh_dist_1st_cla1_cla2		{$sister_ri[0]}{$sister_ri[1]}		= $spin_value_of_quartet_topology{$sorted_scored_top[2]} ;
			}
			else{
				
				$hoh_dist_1st_cla1_cla2		{$sister_le[0]}{$sister_le[1]}		= $spin_value_of_quartet_topology{$sorted_scored_top[$topo_quartet]} ;
				$hoh_dist_1st_cla1_cla2		{$sister_ri[0]}{$sister_ri[1]}		= $spin_value_of_quartet_topology{$sorted_scored_top[$topo_quartet]} ;
			}
			
			$hoh_dist_1st_2nd_cla1_cla2		{$sister_le[0]}{$sister_le[1]}		= $spin_value_of_quartet_topology{$sorted_scored_top[$topo_quartet]} ;
			$hoh_dist_1st_2nd_cla1_cla2		{$sister_ri[0]}{$sister_ri[1]}		= $spin_value_of_quartet_topology{$sorted_scored_top[$topo_quartet]} ;
			push @{$hol_values_of_quartet			{$topology}}				, $spin_value_of_quartet_topology{$sorted_scored_top[$topo_quartet]} ;
			
			if 		( $topo_quartet	==	0	){
				
				push @{$hol_taxvalues_of_subclade{$topology}{$topo_quartet}}		, ( $top_q, $spin_value_of_quartet_topology{$sorted_scored_top[$topo_quartet]} )
			}
			elsif 	( $topo_quartet	==	1	){
			...
			for (0 .. 3){
				...
				push @{$hol_val_of_top_of_tax{$topology}{$taxa[$_]}}				, $spin_value_of_quartet_topology{$sorted_scored_top[$topo_quartet]} ;
			}
		}
		if ( $href_parameter_setting_of_option->{N} == 0 ){ print OUTrej	"\n" }
		################################
		
		#################################################################################################################### END MAIN QUARTET RESULT ASSIGNMENT
	}
	#################################################################################################################################################### END Single Quartet Analyses
	
	
	
	#################################################################################################################### START Result Print OUT
	...
	##############
	# Print rejected quartets at the terminal and in the rejected info file
	if ( %rejected_quartet_of_number ){ 
		
		print "\n\t", $N_rejected_quartets, " Rejected Quartets:"	; for ( sort keys %rejected_quartet_of_number ){ print			"\n\tq"	, $_, ": ", $rejected_quartet_of_number{$_} } print "\n\t"	;
		
		if ( $href_parameter_setting_of_option->{N} == 0 ){
			
			print OUTrej					"\n\nRejected Quartets:"; for ( sort keys %rejected_quartet_of_number ){ print OUTrej	"\nq"	, $_, ": ", $rejected_quartet_of_number{$_} }
		}
	}
	##############
	...
	print	OUTinfo	"\n\tN Single Quartet-Analyses:\t"		, $N_final_quartets	,
					"\n\tMean Remaining Site Positions:\t"	, $mean_remain_pos	,
					"\n\tMean Sites Rejected Quartets:\t"	, $rejected_pos_all	;
	################################
	...
	my @quartets_all											=	sort keys %seen_quartets_all ;
	for ( 0 .. 2 ){ $href_grafik_colours->{$quartets_all[$_]}	=	$href_grafik_colours->{$_} }
	...
	for my $q_number ( 0 .. @$aref_quartets-1 ){
		
		unless ( $rejected_quartet_of_number{$q_number+1} ){
			
			print OUTcla	"Split Analysis q" , $q_number+1, ":\t";
			...
			for my $topology ( 0 .. 2 ){
				...
				$lol_data_ter[$repeat][$topology]	=	$hol_values_of_quartet{$quartets_all[$topology]}[$repeat];	# Assign split score for triangle print out
				push @values,							$hol_values_of_quartet{$quartets_all[$topology]}[$repeat];	# Assign split score for triangle result string
				$sum_score_of_top{$topology}		+=	$hol_values_of_quartet{$quartets_all[$topology]}[$repeat];	# Assign split score for mean split score calculation
				push @{$lol_scores_of_top[$topology]},	$hol_values_of_quartet{$quartets_all[$topology]}[$repeat];	# Assign split score for medain split score calculation
			}
			##############
			
			##############
			# Generate triangle result info string for each quartet run
			($name_of_repeat{$repeat}			=	@$aref_quartets[$q_number])	=~	s/:/,/g ;
	...
	&print_svg_triangle_graphic(
								...
								\%name_of_repeat					,	# key: quartet number (0 to @quartets-1); value: quartet taxa (tax1:tax2:tax3:tax4)										In -> Defined, OUT -> Unchanged
	...
	################################
	# print svg bar plot output of each subclade topology
	unless ( $href_txt_filenames->{barN} eq 'none' ){
		
		my @subheader	= ( "best_supported", "2nd_best_supported" ) ;
		
		for my $topologynumber ( 0 .. 1 ){
			...
			&print_svg_barchart_graphic(
								...
								\@quartets_all						,	# list of sorted clade topologies										In -> Defined, OUT -> Unchanged
								\%hoh_seen_quartet					,	# key: sublade topology; value: N occurence								In -> Defined, OUT -> Unchanged
								\$N_final_quartets					,	# total number of quartets												In -> Defined, OUT -> Unchanged
								\$href_graphic_title->{barN}		,	# svg graphic title														In -> Defined, OUT -> Unchanged
								\$href_txt_filenames->{barN}		,	# svg outfile name														In -> Defined, OUT -> Unchanged
								\$subheader[$topologynumber]			,	# outfile suffix extension												In -> Defined, OUT -> Unchanged
								\$topologynumber					,	# list number of each quartet topology (0,1,2)							In -> Defined, OUT -> Unchanged
								\%$href_graphic_setup				,	# key: svgoption; value: option setup									In -> Defined, OUT -> Unchanged
								\$href_name_subfolder->{svg}		,	# key: subfolder code; value: subfolder name							In -> Defined, OUT -> Unchanged
							);
		}
	...
	################################
	# print svg number split support graphic
	my @subheader	=	( 'Best', 'Second Best' );
	
	for my $support ( 0 .. 1 ){
		
		my %hol_values_of_topol ;
		for my $top ( 0 .. 2 ){ $N_of_top{$quartets_all[$top]}	=	$hoh_seen_quartet{$support}{$quartets_all[$top]} }
		
		@sorted_topologies_by_value	=	sort {$N_of_top{$b}<=>$N_of_top{$a}} keys %N_of_top ;
		for ( 0 .. 2 ){
			
			$hol_values_of_topol{$sorted_topologies_by_value[$_]}	=	$N_of_top{$sorted_topologies_by_value[$_]} / ( $N_of_top{$sorted_topologies_by_value[0]} + $N_of_top{$sorted_topologies_by_value[1]} + $N_of_top{$sorted_topologies_by_value[2]} ) ;
			
			#print "\nsupport ", $support, "\ttopology", $_,": ", $sorted_topologies_by_value[$_], "\t", $hol_values_of_topol{$sorted_topologies_by_value[$_]}
		}
		
		( my	 $name_outfile	=	$href_txt_filenames->{spliN} ) =~ s/.svg$/_${subheader[$support]}.svg/ ;
				 $name_outfile	=~	s/ /_/ ;
				 $name_outfile	=~	s/Best/best/ ;
				 $name_outfile	=~	s/Second/second/ ;
		( my	$name_title		=	$href_graphic_title->{spliN} ) =~ s/Quartet/${subheader[$support]} Quartet/ ;
		
		&print_svg_split_graphic(
								
								\%$href_grafik_colours				,	# key: topology support (0,1, or 2); value: assigned svg color								In -> Defined, OUT -> Unchanged
								\@sorted_topologies_by_value		,	# list of sorted clade topologies															In -> Defined, OUT -> Unchanged
								\$name_outfile						,	# Output svg name																			In -> Defined, OUT -> Unchanged
								\$name_title						,	# Graphic header text																		In -> Defined, OUT -> Unchanged
								\%hol_values_of_topol				,	# key: toponumber (0, 1, or 2); value: mean split score										In -> Defined, OUT -> Unchanged
								\%$href_graphic_setup				,	# key: svgoption; value: option setup														In -> Defined, OUT -> Unchanged
								\%N_of_top							,	# key: toponumber (0, 1, or 2); value: N resolution, only important in	split Number		In -> Defined, OUT -> Unchanged
								\'N Quartet Split'					,	# value code of figure legend print out														In -> Defined, OUT -> Unchanged
								\%q_number_of_top					,	# key alphabetically sorted topology, value: Q code											In -> Defined, OUT -> Unchanged
								\$href_name_subfolder->{svg}		,	# key: subfolder code; value: subfolder name												In -> Defined, OUT -> Unchanged
							 ) ;
	}
	...
	unless ( ( @$aref_quartets == 1 ) || ( $href_txt_filenames->{txtbsupport} eq 'none' ) ){
		
		@subheader	=	( 'Best', 'Second Best' );
		for my $support (0 .. 1){
			...
			&print_txt_quartet_results(
								...
								\$href_txt_filenames->{txtbsupport}	,	# text outfile name																						In -> Defined, OUT -> Unchanged
								\$subheader[$support]				,	# text outfile name suffix																				In -> Defined, OUT -> Unchanged
								\$support							,	# 0 -> best; 1 -> second best																			In -> Defined, OUT -> Unchanged
								\$href_name_subfolder->{txt}		,	# key: subfolder code; value: subfolder name															In -> Defined, OUT -> Unchanged
							) ;
		}
	}
	...
	if ( $href_parameter_setting_of_option->{N} == 0 ){ close	OUTrej	}
...
sub start_phyquart{
	...
	my	$sref_outfile_q_calc					=	$_[8]	;	#	name single quartet calculation															In -> Defined, OUT -> Unchanged
	...
	unless ( $$sref_outfile_q_calc eq 'none' ){
		...
		open 	OUTinfotxt, 	">$$sref_name_folder/$info_t22_out" or die "OUTFILE-ERROR: Cannot open info outfile ", $$sref_name_folder, "/", $info_t22_out, " in subroutine &determine_quartet_topology_11!\n";
	...
	for my $topology ( @$aref_split_topologies ){ for my $pattern ( qw/A B C D E F G H J K L M N I V/ ){
			...
			unless ( $href_hoh_expected_N_of_topo_of_pattern->{$topology}{$pattern} > 0 ){ $href_hoh_expected_N_of_topo_of_pattern	->{$topology}{$pattern} = 0 }
			unless ( $href_hoh_found_N_of_topo_of_pattern	->{$topology}{$pattern} > 0 ){ $href_hoh_found_N_of_topo_of_pattern		->{$topology}{$pattern} = 0 }
		}
	}
	#############################
	...
	$spin_value_of_topology{$split_topology_1."_lr"}	= $spin_topology_1_nb_red - $spin_topology_1_lr_np_red - $spin_topology_1_lr_nk_red_mean ;
	$spin_value_of_topology{$split_topology_1."_rl"}	= $spin_topology_1_nb_red - $spin_topology_1_rl_np_red - $spin_topology_1_rl_nk_red_mean ;
	$spin_value_of_topology{$split_topology_2."_lr"}	= $spin_topology_2_nb_red - $spin_topology_2_lr_np_red - $spin_topology_2_lr_nk_red_mean ;
	$spin_value_of_topology{$split_topology_2."_rl"}	= $spin_topology_2_nb_red - $spin_topology_2_rl_np_red - $spin_topology_2_rl_nk_red_mean ;
	$spin_value_of_topology{$split_topology_3."_lr"}	= $spin_topology_3_nb_red - $spin_topology_3_lr_np_red - $spin_topology_3_lr_nk_red_mean ;
	$spin_value_of_topology{$split_topology_3."_rl"}	= $spin_topology_3_nb_red - $spin_topology_3_rl_np_red - $spin_topology_3_rl_nk_red_mean ;
	#############################
	...
	unless ( $$sref_outfile_q_calc eq 'none' ){
		...
		print OUTinfotxt "\nRemaining apomorph split signal of each spin topology:" ;
	...
	#############################
	# (1)... Identify highest spin signal of each of the three spin quartet topologies
	# (2)... Sort highest split signal found for each quartet in descending order
	# (3)... If highest spin of lowest supported topology < 0, normalize values related to lowest supported quartet
	
	# (1)
	my %highest_spin_of_topology ;
	for my $spin_topo ( @sorted_topologies ){ 
		
		( my $top = $spin_topo )	=~ s/_lr$|_rl$// ;
		
		unless ( $highest_spin_of_topology{$top} ){ $highest_spin_of_topology{$top}	= $spin_value_of_topology{$spin_topo} }	# (1)
	}
	###
	...
	# test print
	#for (@sorted_spin_topos){print "\nhighest spin:", $_, "\t", $highest_spin_of_topology{$_}, "\n" } print "\n"; 
	###
	...
	if ( $highest_spin_of_topology{$sorted_spin_topos[2]} < 0 ){
		...
		unless ( $$sref_outfile_q_calc eq 'none' ){
			...
			print OUTinfotxt	"\n\nBest spin value of lowest supported quartet < 0:\t", $highest_spin_of_topology{$sorted_spin_topos[2]},
								"\nNormalized spin values (+", abs ($highest_spin_of_topology{$sorted_spin_topos[2]}), "):\n"
		}
	...
	#############################
	# Calculation posterior probability for each split signal n (Signal=(n/signal q1+q2+3)
	my $sum_highest_split ;
	for ( 0 .. 2 ){ $sum_highest_split += $highest_spin_of_topology{$sorted_spin_topos[$_]} }
	
	if ( $sum_highest_split >= 0 ){
		
		for ( 0 .. 2 ){
				
			$highest_spin_of_topology{$sorted_spin_topos[$_]}	=	 $highest_spin_of_topology{$sorted_spin_topos[$_]} / $sum_highest_split
		}
	}
	else{ die "\n\tDATA-ERROR: No split signal found in quartet ", $$sref_quartet_taxa, "!\n" }
	#############################
	
	
	unless ( $$sref_outfile_q_calc eq 'none' ){
		
		for ( @sorted_spin_topos ){	print OUTinfotxt "\nBest score of topology:", $_ , " :\t", $highest_spin_of_topology{$_} }
									print OUTinfotxt "\n" ;
	}
	
	for ( @sorted_spin_topos ){		print "\n\tBest score ", $_ , ":\t", $highest_spin_of_topology{$_} }
									print "\n" ;
	###
	...
	#############################
	# determine single spin result string for symetric split order
	my $result_string = $sorted_topologies[0];
	for my $k ( 0 .. 4 ){
		
		if ( $spin_value_of_topology{$sorted_topologies[$k]} > $spin_value_of_topology{$sorted_topologies[$k+1]} ){
			
			$result_string .= " > ".$sorted_topologies[$k+1]
		}
		else{
	...
	unless ( $$sref_outfile_q_calc eq 'none' ){ print 	OUTinfotxt	"\n\nResult string apomorph split signal of spin topologies:\n", $result_string }
	#############################
	
	
	
	#############################
	# determine highest spin result string for symetric split order
	my $result_string_sym = $sorted_spin_topos[0];
	for my $k ( 0 .. 1 ){
		
		if ( $highest_spin_of_topology{$sorted_spin_topos[$k]} > $highest_spin_of_topology{$sorted_spin_topos[$k+1]} ){
			
			$result_string_sym .= " > ".$sorted_spin_topos[$k+1]
		}
		else{
	...
	unless ( $$sref_outfile_q_calc eq 'none' ){
		...
		print 	OUTinfotxt	"\n\nResult string highest apomorph spin split signal found for each quartet topology:\n", $result_string_sym ,
							"\n\nTopology ", $sorted_spin_topos[0], " identified as pot. correct quartet topology using spin value info (symetric)!";
		close	OUTinfotxt
	}
	#############################
	...
	################################
	# If third highest split value positive or negative, set third value to null and
	# adapt both other values
		$href_spin_value_of_quartet_topology->{$sorted_spin_topos[0]}	=	$highest_spin_of_topology{$sorted_spin_topos[0]} ;
		$href_spin_value_of_quartet_topology->{$sorted_spin_topos[1]}	=	$highest_spin_of_topology{$sorted_spin_topos[1]} ;
		$href_spin_value_of_quartet_topology->{$sorted_spin_topos[2]}	=	$highest_spin_of_topology{$sorted_spin_topos[2]} ;
	################################
	
	
	
	######### 4BASAL print out ONLY if Outfilename is not 'none' (see header outfile declaration) !!!!!!
	unless ( $$sref_out_symetric eq 'none'){
		
		open	OUTpipe,	">>$$sref_name_folder/$$sref_out_symetric" or die "OUTFILE-ERROR: Cannot open info outfile ", $$sref_name_folder, "/", $$sref_out_symetric ," in subroutine &determine_quartet_topology_11!\n" ;
		print	OUTpipe		"Split Analysis q", $$sref_filename_in, ":\t", $$sref_quartet_taxa ,"\t", 
							$sorted_spin_topos[0], "\t", $href_spin_value_of_quartet_topology->{$sorted_spin_topos[0]}, "\t",
							$sorted_spin_topos[1], "\t", $href_spin_value_of_quartet_topology->{$sorted_spin_topos[1]}, "\t",
							$sorted_spin_topos[2], "\t", $href_spin_value_of_quartet_topology->{$sorted_spin_topos[2]}, "\n",
		;
		close	OUTpipe
	}
	#######################
...
sub print_svg_triangle_graphic{
	...
	################################################### START triangle
	# print hypotenuse
	print OUTsvg	"\n\t<line x1=\""			, $x_y_corner1[0]		, "\" y1=\""		, $x_y_corner1[1]		, "\" x2=\"", $x_y_corner2[0]	, "\" y2=\"", $x_y_corner2[1], 
					"\" style=\"stroke-width:"	, $stroke_width_branches, ";stroke:"		, $stroke_color_branches, ";fill:"	, $axis_fill_color	,
					";stroke-opacity:"			, $axis_stroke_opacity	, ";fill-opacity:"	, $axis_fill_opacity	, "\"/>" ;
	
	# print cathetus
	print OUTsvg	"\n\t<line x1=\""			, $x_y_corner1[0]		, "\" y1=\""		, $x_y_corner1[1]		, "\" x2=\"", $x_y_corner3[0]	, "\" y2=\"", $x_y_corner3[1], 
					"\" style=\"stroke-width:"	, $stroke_width_branches, ";stroke:"		, $stroke_color_branches, ";fill:"	, $axis_fill_color	,
					";stroke-opacity:"			, $axis_stroke_opacity	, ";fill-opacity:"	, $axis_fill_opacity	, "\"/>" ;
	
	# print acathetus
	print OUTsvg	"\n\t<line x1=\""			, $x_y_corner2[0]		, "\" y1=\""		, $x_y_corner2[1]		, "\" x2=\"", $x_y_corner3[0]	, "\" y2=\"", $x_y_corner3[1], 
					"\" style=\"stroke-width:"	, $stroke_width_branches, ";stroke:"		, $stroke_color_branches, ";fill:"	, $axis_fill_color	,
					";stroke-opacity:"			, $axis_stroke_opacity	, ";fill-opacity:"	, $axis_fill_opacity	, "\"/>" ;
	################# END triangle
	...
	print OUTsvg	"\n\t<text x=\""	, $x_y_corner3[0]-$text_font_size*0.3	,"\" y=\""						, $x_y_corner3[1]-$text_font_size*0.25	,
					"\" style=\"font-size:"		, $font_size, ";font-family: ", $text_font_family, "; stroke-width:", $text_stroke_width	,";\" ",
	...
	for (0 .. @$aref_lol_data-1){
		...
		print OUTsvg	"\n\t<circle cx=\""	, $lol_x_y[$_][0]		, "\" cy=\""			, $lol_x_y[$_][1]	,
						"\" r=\""			, $circle_radius		,
						"\" stroke=\""		, $circle_stroke_color	, "\" stroke-width=\""	, $circle_stroke_width	,
						"\" onmouseover=\""	;
	...
	for (0 .. 3){
		...
		print OUTsvg	"\n\t<text id=\"spktText", $_ ,"\" x=\""	, $12*$zoom_factor	,"\" y=\""						, $x_y_corner1[1]+$linespace	,
	...
	################################################### Start Title Print
	# Print title
	print OUTsvg	"\n\t<text x=\""	, $x_y_corner1[0]-0.125*$zoom_factor		,"\" y=\""						, $x_y_corner3[1]-$text_font_size	,
...
sub print_svg_split_graphic{
	...
	################################################### Start Title Print
	# Print title
	print OUTsvg	"\n\t<text x=\"35\" y=\"35\" style=\"font-size:12;font-family: Arial; stroke-width:1; fill:black\" > ", $$sref_graphic_title ," </text>" ;
	################################################### End Title Print
...
sub print_svg_barchart_graphic{
	...
	my	$sref_title				=	$_[6]	;	#	outfile suffix extension										In -> Defined, OUT -> Unchanged
	my	$sref_score_number		=	$_[7]	;	#	list number of each quartet topology (0,1,2)					In -> Defined, OUT -> Unchanged
	my	$href_graphic_setup		=	$_[8]	;	#	key: svgoption; value: option setup								In -> Defined, OUT -> Unchanged
	my	$sref_name_folder		=	$_[9]	;	#	key: subfolder code; value: subfolder name						In -> Defined, OUT -> Unchanged
	...
	my $header				= $$sref_title ;																	# Header Text
	...
	my $length_x_axis		= $axis_x_start  + 3 * $width_bar_plot + 3 * $width_bar_space + $axis_space_end ;		# length x axis
	...
	my $docu_height			= 400 ;																				# document height
	...
	my	%colour_of_quartet ;	# key: quartet topology; value: grafik colour
		$colour_of_quartet{$aref_quartets_all->[0]}	= $href_colours_grafik->{$aref_quartets_all->[0]} ;
		$colour_of_quartet{$aref_quartets_all->[1]}	= $href_colours_grafik->{$aref_quartets_all->[1]} ;
		$colour_of_quartet{$aref_quartets_all->[2]}	= $href_colours_grafik->{$aref_quartets_all->[2]} ;
	##############################
	...
	for my $topology ( @$aref_quartets_all ){
		
		if ( $href_hoh_seen_quartet->{$$sref_score_number}{$topology} ){
			
			$bar_plot_y_of_x{$bar_plot_start_x}	= $axis_y_start - (( $href_hoh_seen_quartet->{$$sref_score_number}{$topology} / $N_quartets ) * $high_bar_plot) ;		# y value of each bar plot (%)
			$valu_of_x_start{$bar_plot_start_x}	= $href_hoh_seen_quartet->{$$sref_score_number}{$topology} ;															# assign number of oberved quartet topologies to bar start point
		}
		...
		$colour_of_x_start{$bar_plot_start_x}	= $colour_of_quartet{$topology};								# assign bar plot colour for given topology
		$topo_of_x_start{$bar_plot_start_x}		= $topology ;													# assign quartet topology to bar start point
	...
	my	$outfile_suffix		=	$$sref_title.".svg" ;
		$outfile			=~	s/.svg$/_${outfile_suffix}/ ;
	...
	for my $x ( sort {$a<=>$b} keys %bar_plot_y_of_x ){
		...
		## X legend
		my $legend_text = "Q".$counter ;
		print OUTsvg	"\n\t<text x=\"", $x, "\" y=\"", $y_legend_x_axis, "\" style=\"font-size:"		, $font_size, ";font-family: ", $text_font_family, "; stroke-width:", $text_stroke_width	,";\" onmouseover=\"evt.target.ownerDocument.getElementById('spktText0').firstChild.nodeValue= '", $topo_of_x_start{$x} ,"';evt.target.setAttributeNS(null, 'fill', 'grey');\" onmouseout=\"evt.target.ownerDocument.getElementById('spktText0').firstChild.nodeValue= ' ';evt.target.setAttributeNS(null, 'fill', 'black');\" > ", $legend_text ," </text>" ;
	...
	##############################
	# print svg header
	my	@title_parts	=	split "_", $$sref_title ;
	for	my $part ( @title_parts ){
		
		$part	=~	s/best/Best/g ;
		$part	=~	s/supported/Supported/g ;
	}
	my $bartitle	=	join " ", @title_parts ;
	...
	################################################### Start Title Print
	# Print title
	print OUTsvg	"\n\t<text x=\"", $x_value ,"\" y=\"", $y_value_header-24 ,"\" style=\"font-size:12;font-family: Arial; stroke-width:1; fill:black\" > ", $$sref_graphic_title ," </text>" ;
	################################################### End Title Print
...
sub print_txt_quartet_results{
	...
	my	$quartet_support				=	$_[2]	;	#	key for print out (either 'best' or 'second best' resolved quartets							In -> Defined, OUT -> Unchanged
	my	$sref_support					=	$_[3]	;	#	key: toponumber (0, 1, or 2); value: mean split score										In -> Defined, OUT -> Unchanged
	my	$sref_name_folder				=	$_[4]	;	#	key: subfolder code; value: subfolder name													In -> Defined, OUT -> Unchanged
	
	######################################
	# Print OUT result txt info about resolved quartets
	my	$outfile		=	$$sref_outfile_split_txt	;
	my	$outfile_suffix	=	$$quartet_support.".txt"	;
		$outfile		=~	s/.txt/_${outfile_suffix}/	;
	...
	for my $subclade ( sort keys %$href_hol_taxvalues_of_subclade ){ #print "\nsubclade: ", $subclade;
		
		print OUTtxt	"Quartet topology ", $subclade ," resolved as ", $$quartet_support ," supported quartet by:\n",
						"--------------------------------------------------------------------------\n",
						"Quartet Combination\tSplit Score\n"
		;
		
		my @collected_quartet_value ;
		if ( $href_hol_taxvalues_of_subclade->{$subclade}{$$sref_support} ){
			
			@collected_quartet_value = @{$href_hol_taxvalues_of_subclade->{$subclade}{$$sref_support}} ;
			
			while ( @collected_quartet_value ){
				
				my $quartet	= shift @collected_quartet_value ; #print "\nquartet ", $quartet;
				my $value	= shift @collected_quartet_value ; #print "\nvalue ", $value;
				
				print OUTtxt $quartet, "\t", $value, "\n"
			}
		}
