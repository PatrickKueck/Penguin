...
my %default_parameter_setting = (
						...
						'k' => 0			, # def: 0			step of split score malization	skip from 0 -> normalization by setting the lowest of all 6 spin dependent split scores to zero to 1 -> normalization by setting the lowest of the three best spin dependent scores to zero (in subroutine &start_phyquart)
					) ;
...
sub perform_quartet_analyses{
	...
	for my $q_number ( 0 .. @$aref_quartets-1 ){
		...
		unless ( $rejected_quartet_of_number{$q_number+1} ){
			...
			if ( $href_txt_filenames->{txtqsupport} ne 'none'){ print OUTcla	"Split Analysis q" , $q_number+1, ":\t" }
		...
		else { print OUTcla	"Split Analysis q", $q_number+1, ":\tQuartet sequence length below allowed threshold -> rejected\n" }
	}
	...
	unless ( $href_txt_filenames->{triall} eq 'none' ){
		...
		&print_svg_triangle_graphic(
								...
								\$href_grafik_colours->{header}		,	# header color
		)
	}
		@lol_data_ter	= 	()	;
	################################
...
sub non_directive_clan_evaluation{
	...
	for my $quartet_number ( 1 .. @$aref_split_topologies ){ #print "\nq number: ", $quartet_number;
		...
		unless ( $href_rejected_quartet_of_number->{$quartet_number} ){
			...
			# (2) subsequent normalization in respect of the lowest best spin toopology score (set to zero) if -k option equal 1
			...
			if ( $$sref_norm_level == 0 ){
				...
				if ( $highest_spin_of_topology{$sorted_spin_topos[2]} > 0 ){
					...
					$highest_spin_of_topology{$sorted_spin_topos[0]} -= $highest_spin_of_topology{$sorted_spin_topos[2]} ;
					$highest_spin_of_topology{$sorted_spin_topos[1]} -= $highest_spin_of_topology{$sorted_spin_topos[2]} ;
					$highest_spin_of_topology{$sorted_spin_topos[2]} -= $highest_spin_of_topology{$sorted_spin_topos[2]}
				}
			...
			unless ( $$sref_out_symetric eq 'none'){
				...
				print	OUTpipe		$$sref_msa_infile_name, "\t", $quartet_number+1, ":\t", $aref_split_topologies->[$quartet_number],"\t",
...
sub spin_directive_clan_evaluation{
	...
	for my $spin_topo ( @sorted_spin_topos ){
		...
		#else { delete $href_sum_value_of_spin_dir->{$spin_topo}; delete $href_spin_value_of_quartet_topology->{$spin_topo} }
	...
	#############################
	# (1) subsequent normalization in respect of the lowest best spin toopology score (set to zero) if -k option equal 1
		...
		unless ( $href_rejected_quartet_of_number->{$quartet_number} ){
			...
			if ( $$sref_norm_level == 0 ){
				...
				if ( $value_of_top{$sorted_spin_topos[2]} > 0 ){
					...
					$href_spin_value_of_quartet_topology->{$sorted_spin_topos[0]}[$counter_quartet] -= $value_of_top{$sorted_spin_topos[2]} ;
					$href_spin_value_of_quartet_topology->{$sorted_spin_topos[1]}[$counter_quartet] -= $value_of_top{$sorted_spin_topos[2]} ;
					$href_spin_value_of_quartet_topology->{$sorted_spin_topos[2]}[$counter_quartet] -= $value_of_top{$sorted_spin_topos[2]}
				}
